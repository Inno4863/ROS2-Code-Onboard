...
class ControlServerNode(Node):
    def __init__(self):
        ...
        #self.turn_service = self.create_service(Comms, 'turn_right', self.turn_callback)
        ...
        self.turnleft_server = self.create_service(Trigger, 'turn_left', self.turnleft_callback)
        self.turnright_server = self.create_service(Trigger, 'turn_right', self.turnright_callback)
    ...
    async def turnright_callback(self, request, response):
        self.get_logger().info('Turning right')
        self.loop.run_until_complete(self.perform_turn(self.latitude, self.longitude, self.altitude, +30.0))
        response.success = True
        return response

    async def turnleft_callback(self, request, response):
